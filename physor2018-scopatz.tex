...
\begin{abstract}
This paper presents a binary formaulation of the Bateman equations
for radioactive decay. This work includes various limiting cases
that arrise in real-world calculations due to ill-behaved nuclear data
...
this paper presents a verification study tha compares the binary solver
to the Chebychev Rational Approximation Method (CRAM). The comparion
between CRAM and the solver descibed demonstrates that the method is
...
\cite{scopatz2015decay}, the formuation presented here aims to remove the
assumption that the nuclear data is well-behaved.

The fundemntal nuclear data is not well-behaved. There are degeneracies in
...
The binary formualtion has computaional speed advanatages over the
traditional method. \S\ref{sec-impl-spec-approx} the presents the implemntation
...
\begin{figure}[!htb]
  ...
  \caption{A histogram of the absolute value of the differences between the
           ...
           from an intial \ith nuclide to a \jth nuclide for all times.}
  \label{fig-diff-hist}
\end{figure}
...
solver. This occationally leads to catastrophic floating point cancelation
...
method minimzies run time arithmetic operations and takes advantage of
...
approximations involve inverting many matricies. CRAM, thus, requires many
...
decay chain in which these issues arrise.
